...
import { Vector3, Matrix4, Object3D, ObjectLoader, Quaternion , PerspectiveCamera, BoxGeometry, MeshBasicMaterial, Mesh} from "three";
import { Engine } from "./engine";
import { MS_TO_SECONDS, GRAVITY, RAD_TO_DEG } from "../constants/car-constants";
...
const COLLISION_LENGHT: number = 3;
const COLLISION_HEIGHT: number = 2;
const COLLISION_WIDTH: number = 1.5;
// tslint:disable-next-line:no-magic-numbers
const ROTATION: number = (Math.PI / 2) * 3;

export abstract class Car extends Object3D {
    ...
    protected async load(): Promise<Object3D> {
    ...
    protected createCollisionBox(): void {
        const geometry: BoxGeometry = new BoxGeometry(COLLISION_WIDTH, COLLISION_HEIGHT, COLLISION_LENGHT);
        const material: MeshBasicMaterial = new MeshBasicMaterial( {visible: false} );
        const cube: Mesh = new Mesh( geometry, material );
        this.mesh.add(cube);
    }

    public abstract async init(positionMatrix: Matrix4): Promise<void>;

    protected adjustPosition(positionMatrix: Matrix4): void {
        this.mesh.applyMatrix(positionMatrix);
    ...
    public getCarDirection(): Vector3 {
        const rotationMatrix: Matrix4 = new Matrix4();
        rotationMatrix.extractRotation(this.mesh.matrix);
        const rotationQuaternion: Quaternion = new Quaternion();
        rotationQuaternion.setFromRotationMatrix(rotationMatrix);

        return new Vector3(0, 0, -1).applyMatrix4(rotationMatrix);
    }
    ...
    public cameraLink(camera: PerspectiveCamera, isAdding: boolean): void {
        if (isAdding) {
